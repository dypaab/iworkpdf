...
async function doLogin(){
  ...
  try{
    ...
    if(error){
      ...
      await sb.from('audit_logs').insert({user_id:null,action:'login_failed',metadata:{email,ua:navigator.userAgent.substring(0,80)}}).catch(()=>{});
...
async function dlSigned(path,filename,fileId){
  ...
  try{
    ...
    await sb.rpc('increment_downloads',{file_id:fileId}).catch(()=>{});
