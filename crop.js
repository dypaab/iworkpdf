...
function buildCropUI(){
  ...
  return `
    ...
    <div class="crop-preview" id="crop-prev">
      <div style="width:80%;height:80%;background:#e8e8e8;border-radius:4px;position:relative">
        <div class="crop-inner" id="crop-inner" style="inset:12%;"></div>
      </div>
    </div>
    ${saveBlock}${bottom}
...
function updateCropPreview(){
  ...
  if(!inner) return;
  const top=Math.min(45,Math.max(0,parseInt(document.getElementById('crop-top')?.value||'10')/2));
  const bot=Math.min(45,Math.max(0,parseInt(document.getElementById('crop-bottom')?.value||'10')/2));
  const lft=Math.min(45,Math.max(0,parseInt(document.getElementById('crop-left')?.value||'10')/2));
  const rgt=Math.min(45,Math.max(0,parseInt(document.getElementById('crop-right')?.value||'10')/2));
  inner.style.top=top+'%';
  inner.style.bottom=bot+'%';
  inner.style.left=lft+'%';
  inner.style.right=rgt+'%';
}
